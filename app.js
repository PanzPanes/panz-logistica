...
async function sbGet(table, query='') { return sb(table + (query?'?'+query:'')); }
async function sbPost(table, body) { return sb(table, { method:'POST', body:JSON.stringify(body) }); }
async function sbPatch(table, query, body) { return sb(table+'?'+query, { method:'PATCH', body:JSON.stringify(body) }); }
async function sbDelete(table, query) { return sb(table+'?'+query, { method:'DELETE', prefer:'' }); }
...
function optimizar(lista){
  ...
  while(rest.length){
    ...
    rest.forEach((l,i)=>{const km=dist(actual,l),t=Math.round(km/25*60)+7,llega=mins+t,abre=tMin(l.abre),cierra=tMin(l.cierra),espera=llega<abre?abre-llega:0,pen=llega>cierra?9999:0;if(km+espera*.5+pen<bs){bs=km+espera*.5+pen;bi=i;}});
    const sig=rest[bi],km=dist(actual,sig),t=Math.round(km/25*60)+7;
    mins+=t;const llegaReal=Math.max(mins,tMin(sig.abre));
...
function generarVueltas(){
  ...
  const lista=ids.map(id=>{const loc=locales.find(l=>l.id===id);return{...loc,cajones:totalCajones(pedidos[id]),pedido:pedidos[id]};});
  vueltas=[];let rest=[...lista];
  while(rest.length){
    let vuelta=[],cap=0;
    ...
    const usados=new Set(vuelta.map(l=>l.id));rest=rest.filter(l=>!usados.has(l.id));
  }
  entregas={};tiempoReal=null;guardarDia();renderDia();
}
...
function renderDia(){
  ...
  document.getElementById('exp-alertas').innerHTML=totalCaj>CAP?`<div class="alert alert-w"><i class="ti ti-alert-triangle"></i> ${totalCaj} cajones — necesitás ${nv} vueltas</div>`:'';
  const vc=document.getElementById('vueltas-exp');vc.innerHTML='';
  if(vueltas.length){vueltas.forEach((v,i)=>{const h=v.paradas.filter(p=>entregas[p.id]).length,pct=Math.round(h/v.paradas.length*100);vc.innerHTML+=`<div class="vuelta-hdr"><div><div class="vuelta-title"><i class="ti ti-truck"></i> Vuelta ${i+1}</div><div class="vuelta-sub">${v.paradas.length} paradas · ${v.cajones} cajones</div></div><span class="badge badge-o">${pct}% entregado</span></div><div class="progress-wrap"><div class="progress-bar" style="width:${pct}%"></div></div>`;});}
...
function togglePedido(id){if(pedidos[id]){delete pedidos[id];}else{const v={};productos.forEach(p=>v[p.id]=0);pedidos[id]=v;}guardarDia();renderDia();}
function setCajon(id,pid,val){if(!pedidos[id]){const v={};productos.forEach(p=>v[p.id]=0);pedidos[id]=v;}pedidos[id][pid]=parseInt(val)||0;document.getElementById('st-caj').textContent=Object.keys(pedidos).reduce((s,i)=>s+totalCajones(pedidos[i]),0);guardarDia();}
...
function renderLocales(){
  ...
  lista.innerHTML=locales.map(l=>`<div class="card"><div class="card-nombre">${l.nombre}</div><div class="card-sub"><i class="ti ti-map-pin"></i> ${l.dir}</div><div class="card-sub"><i class="ti ti-phone"></i> ${l.tel}</div><div class="card-sub"><i class="ti ti-clock"></i> ${l.abre}–${l.cierra}</div>${l.deuda>0?`<div style="margin-top:6px"><span class="badge badge-r">${l.deuda} cajones adeudados</span></div>`:''}<div class="card-actions"><button class="btn-sm" onclick="abrirModalLocal(${l.id})"><i class="ti ti-edit"></i> Editar</button><button class="btn-sm btn-danger" onclick="eliminarLocal(${l.id})"><i class="ti ti-trash"></i></button></div></div>`).join('');
}

function abrirModalLocal(id){
  if(id){const l=locales.find(x=>x.id===id);document.getElementById('ml-title').textContent='Editar local';document.getElementById('ml-id').value=id;document.getElementById('ml-nombre').value=l.nombre;document.getElementById('ml-dir').value=l.dir;document.getElementById('ml-tel').value=l.tel||'';document.getElementById('ml-abre').value=l.abre;document.getElementById('ml-cierra').value=l.cierra;document.getElementById('ml-deuda').value=l.deuda||0;}
  else{document.getElementById('ml-title').textContent='Nuevo local';document.getElementById('ml-id').value='';['ml-nombre','ml-dir','ml-tel'].forEach(f=>document.getElementById(f).value='');document.getElementById('ml-abre').value='08:00';document.getElementById('ml-cierra').value='14:00';document.getElementById('ml-deuda').value='0';}
...
async function guardarLocal(){
  ...
  const d={nombre,dir,tel:document.getElementById('ml-tel').value,abre:document.getElementById('ml-abre').value,cierra:document.getElementById('ml-cierra').value,deuda:parseInt(document.getElementById('ml-deuda').value)||0,lat:-31.41+(Math.random()-.5)*.06,lng:-64.19+(Math.random()-.5)*.06};
  ...
  cerrarModal('local');toast('Local guardado');renderLocales();renderDia();
}
...
function renderRepartidor(){
  ...
  if(!vueltas.length){hdr.innerHTML='';prog.innerHTML='';par.innerHTML='<div class="empty">Expedición aún no generó las vueltas del día.<br><br>Tileá los locales y tocá "Generar vueltas".</div>';return;}
  const v=vueltas[vueltaActiva],h=v.paradas.filter(p=>entregas[p.id]).length,pct=Math.round(h/v.paradas.length*100);
  ...
  par.innerHTML=v.paradas.map((p,i)=>{
    const done=entregas[p.id],ent=done||{},mapsUrl=`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(p.dir)}`;
    ...
    return `<div class="parada-row" style="${done?'opacity:.6':''}"><div class="parada-num ${done?'done':''}">${done?'<i class="ti ti-check"></i>':i+1}</div><div class="parada-body"><div class="parada-nombre">${p.nombre}</div><div class="parada-sub"><i class="ti ti-map-pin"></i> ${p.dir}</div><div class="parada-sub"><i class="ti ti-phone"></i> <a href="tel:${p.tel}" style="color:var(--o);font-weight:600">${p.tel}</a></div><div class="parada-sub"><i class="ti ti-clock"></i> ${p.abre}–${p.cierra}</div>${done?`<div style="margin-top:6px;font-size:13px;color:var(--v);font-weight:500"><i class="ti ti-check"></i> Entregados: ${totalEnt} · Recuperados: ${totalRec}</div>`:''}<div class="card-actions" style="margin-top:10px">${!done?`<button class="btn-v btn-sm" onclick="abrirEntrega(${p.id})"><i class="ti ti-package"></i> Registrar entrega</button>`:''}<a href="${mapsUrl}" target="_blank"><button class="btn-sm"><i class="ti ti-map-2"></i> Abrir Maps</button></a></div></div><div style="text-align:right;flex-shrink:0"><div class="parada-hora">${p.llegada}</div><div style="font-size:12px;color:var(--text2)">${p.km} km</div></div></div>`;
  }).join('');
}
...
function abrirEntrega(localId){
  ...
  const mkGrid=(prefijo,vals)=>productos.map(p=>`<div class="caja-wrap"><div class="caja-label" style="color:${p.color}">${p.nombre}</div><input class="caja-input" type="number" min="0" id="${prefijo}-${p.id}" value="${vals[p.id]||0}"></div>`).join('');
...
async function confirmarEntrega(){
  ...
  const totalEnt=Object.values(entregados).reduce((s,v)=>s+v,0),totalRec=Object.values(recuperados).reduce((s,v)=>s+v,0);
...
async function cerrarVuelta(){
  ...
  const v=vueltas[vueltaActiva],ultima=v.paradas[v.paradas.length-1],durEst=ultima?ultima.minFinal-660+15:0;
...
function renderMetricas(){
  ...
  document.getElementById('m-ent').textContent=ent;document.getElementById('m-rec').textContent=rec;
  const v=vueltas.length&&vueltas[0].paradas.length?vueltas[0].paradas[vueltas[0].paradas.length-1]:null,est=v?v.minFinal-660+15:0;
  document.getElementById('m-test').textContent=est||'—';document.getElementById('m-treal').textContent=tiempoReal||'—';
  if(tiempoReal&&est)document.getElementById('m-efic').textContent='Eficiencia: '+Math.round(est/tiempoReal*100)+'%';
  const deudas=locales.filter(l=>l.deuda>0);
  document.getElementById('m-deudas').innerHTML=deudas.length?deudas.map(l=>`<div class="card" style="display:flex;justify-content:space-between;align-items:center"><div><div class="card-nombre">${l.nombre}</div><div class="card-sub">${l.tel}</div></div><span class="badge badge-r">${l.deuda} cajones</span></div>`).join(''):'<div class="empty" style="padding:20px">Sin adeudos pendientes</div>';
}
