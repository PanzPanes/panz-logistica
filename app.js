...
const FABRICA = { lat: -31.4135, lng: -64.1934, nombre: 'Panz — Fábrica' };
...
async function geocodificar(direccion) {
  try {
    // Agregamos ", Argentina" para mejorar precisión
    ...
    const res = await fetch(url, {
      headers: { 'Accept-Language': 'es', 'User-Agent': 'PanzLogistica/1.0' }
    });
...
function optimizar(lista) {
  ...
  let rest=[...lista], ruta=[], actual=FABRICA, mins=0;
  while(rest.length){
...
function renderDia(){
  ...
  if(vueltas.length){
    vueltas.forEach((v,i)=>{
      const h=v.paradas.filter(p=>entregas[p.id]).length,pct=Math.round(h/v.paradas.length*100);
  ...
  locales.forEach(loc=>{
    ...
    const sinCoords=!loc.lat||Math.abs(loc.lat+31.41)<0.001;
    lista.innerHTML+=`<div class="card" style="${act?'border-color:var(--o);background:var(--ol)':''}">
      <div style="display:flex;align-items:flex-start;gap:10px">
        ...
        <div style="flex:1">
          ...
          ${sinCoords?`<span class="badge badge-r" style="margin-top:4px;display:inline-block"><i class="ti ti-alert-triangle"></i> Sin coordenadas — editá para actualizar</span>`:''}
...
function renderLocales(){
  ...
  lista.innerHTML=locales.map(l=>{
    const coordOk=l.lat&&Math.abs(l.lat+31.41)>0.001;
    return `<div class="card">
      ...
      <div class="card-sub" style="color:${coordOk?'var(--v)':'#BA7517'}">
        <i class="ti ti-${coordOk?'map-check':'alert-triangle'}"></i>
        ${coordOk?'Ubicación GPS confirmada':'Sin coordenadas GPS — editá para actualizar'}
      </div>
...
async function guardarLocal(){
  ...
  // Mostrar que está geocodificando
  const btnGuardar=document.querySelector('#modal-local .btn-p');
  ...
  if(id){
    await sbPatch('locales','id=eq.'+id,d);
    locales=locales.map(l=>l.id===Number(id)?{...l,...d}:l);
  } else {
    const res=await sbPost('locales',d);
    if(res&&res.length)locales.push(res[0]);
  }

  btnGuardar.textContent='Guardar';
  btnGuardar.disabled=false;
  cerrarModal('local');
  toast(coords.ok?'Local guardado con GPS ✓':'Local guardado (dirección no encontrada)');
  renderLocales();renderDia();
}
...
function selColor(c){
  ...
  COLORES.forEach(col=>{const el=document.getElementById('cp-'+col.replace('#',''));if(el)el.style.border=col===c?'3px solid #1a1a1a':'3px solid transparent';});
}
...
async function confirmarEntrega(){
  ...
  if(idx>=0){
    locales[idx].deuda=Math.max(0,(locales[idx].deuda||0)+diff);
    await sbPatch('locales','id=eq.'+id,{deuda:locales[idx].deuda});
  }
